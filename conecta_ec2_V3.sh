...
function listar_instancias {
    ...
    echo ""
    echo -e "${Blue}F => Para limpar o histórico de buscar EC2 ☁️${Color_Off}"
    echo ""
    echo -e "${BYellow} --->   Para fechar a aplicação pressione CRTL+C <--- ${Color_Off}"
    echo ""
    echo -e "${Blue}F => Para limpar o histórico de buscar EC2 ☁️${Color_Off}"
}
...
if [ "$selection_history" == 'f' ] || [ "$selection_history" == 'F' ]
then
    limpar_historico
    listar_instancias
